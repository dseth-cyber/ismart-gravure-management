...
class NotificationPrefService {
    static async getUserPrefs(userId) {
        return database_1.prisma.userNotificationPref.findMany({ where: { userId } });
    }
    static async upsert(data) {
        return database_1.prisma.userNotificationPref.upsert({
    ...
    static async bulkUpsert(userId, prefs) {
        const results = [];
    ...
    static async remove(userId, channel) {
        return database_1.prisma.userNotificationPref.delete({ where: { userId_channel: { userId, channel } } });
    }
    static isInQuietHours(pref) {
