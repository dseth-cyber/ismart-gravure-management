...
export function HistogramChart({ data, height = '100%' }: Props) {
    ...
    <div style={{ width: '100%', height }} className="flex-1 min-w-0 relative w-full h-full flex flex-col justify-center">
      ...
      <ResponsiveContainer width="100%" height="100%">
        <RechartsBar data={data} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
          ...
          <XAxis dataKey="label" tick={{ fontSize: 9 }} stroke="currentColor" className="opacity-40" />
          ...
          />
          <Bar dataKey="value" fill="#a855f7" radius={[0, 0, 0, 0]} />
        </RechartsBar>
      </ResponsiveContainer>
    </div>
  );
